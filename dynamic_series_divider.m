...
selected_series=18:1:57;

max_slice_location=-4;
min_slice_location=-85;
...
for i=3:count_of_files
    ...
    if ~isempty(find(selected_series==round(dicom_info.SeriesNumber))) % choose series
         ...
         series_number = num2str(dicom_info.SeriesNumber,'%05u');
         series_file_number = num2str(dicom_info.InstanceNumber,'%05u');
 
    
         time_step_i_folder = strcat('time_step_',series_file_number);
    
 
         dicom_file_name = strcat(series_number,series_file_number);
    
         dicom_file_path = strcat(target_folder_name,'\',time_step_i_folder);
